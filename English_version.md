...
Our goal will be to find all city names, codes of individual cities, but also links that are hidden under the character X. We will find all this data using the variables we
...
This part of code is hidden in the ```get_first_columns``` function.
...
However, the links we found are incomplete because everyone is missing the main part. In line 6 we could notice the variable ```MAIN_URL```, in which the main part of 
